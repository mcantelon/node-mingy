...
exports.Command.prototype = {
  ...
  try: function(type_handlers, env, lexemes, test_mode) {
    ...
    if (this.syntax) {
      for(var index in this.syntax) {
        ...
        if (syntax_lexemes.length == lexemes.length) {
          ...
          if (valid) {
            var result = this.determine_command_arguments(type_handlers, syntax_lexemes, lexemes)
            if (result['error']) {
              valid = false
              error = result['error']
            }
  ...
  try_syntax_keywords: function(syntax_lexemes, submitted_lexemes) {
    ...
    for (var index in syntax_lexemes) {
      ...
      if (!this.case_sensitive) {
        syntax_lexeme = syntax_lexeme.toLowerCase()
        submitted_lexeme = submitted_lexeme.toLowerCase()
      }
  ...
  determine_command_arguments: function(type_handlers, syntax_lexemes, input_lexemes) {
    ...
    for (var index in syntax_lexemes) {
      ...
      if (lexeme[0] == '<') {
        ...
        if (type_handlers[reference_type]) {

          if (typeof lexemes[lexeme_to_test] == reference_type) {
            arg[reference_name] = type_handlers[reference_type](lexemes[lexeme_to_test])
          }
          else {
            error = 'Error'
          }
