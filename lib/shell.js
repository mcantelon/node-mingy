...
exports.Shell.prototype = {
  ...
  defaultMode: function(shell, data, stream) {
    ...
    if (shell.streamActive(stream)) {
      var output = shell.parser.parse(data, false, stream)
  ...
  main: function(parser, shell, stream, isStream) {
    ...
    stream.on('connect', function() {
      ...
      if (shell.connectLogic) {
        var connectOutput = shell.connectLogic(shell, stream)
        if (connectOutput) {
  ...
  execute: function(parser, shell, data, stream) {
    ...
    if (shell.mode && shell.modes[shell.mode]) {
      var modeOutput = shell.modes[shell.mode](shell, data, stream)
      output += (modeOutput) ? modeOutput : ''
    }

    if (shell.logic) {
      var postCommandOutput = shell.logic(shell, stream)
